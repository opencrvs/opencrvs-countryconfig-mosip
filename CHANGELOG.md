...
- **MOSIP integration registered on startup with seeded credentials**: unlike the reference configuration described above, this one does **not** ship `INTEGRATIONS` empty — it holds a `MOSIP` entry with the `record.register` scope for birth and death, so the integration is created on every startup. Set `MOSIP_INTEGRATION_CLIENT_ID` (a UUID) and `MOSIP_INTEGRATION_CLIENT_SECRET` and user management seeds the integration with exactly those values, so mosip-api authenticates with the same client id and secret it carries in its own env and no manual "Refresh secret" step is needed. Leave them empty to fall back to generated credentials revealed under Configurations → Integrations. Both are wired in `infrastructure/docker-compose.deploy.yml` from `OPENCRVS_MOSIP_CLIENT_ID` / `OPENCRVS_MOSIP_CLIENT_SECRET`, which also populate mosip-api's `OPENCRVS_CLIENT_ID` / `OPENCRVS_CLIENT_SECRET`.

  **This requires mosip-api `1.9.2-mock.2` or later.** Earlier images have no `OPENCRVS_AUTH_URL` / `OPENCRVS_CLIENT_ID` / `OPENCRVS_CLIENT_SECRET` support and confirm registrations with the requesting user's token instead, so the seeded credentials would go unused. Set `MOSIP_API_VERSION` accordingly. [#12360](https://github.com/opencrvs/opencrvs-core/issues/12360)

- **e-Signet can now be turned off for a single page**: the e-Signet and QR fields themselves are unchanged; what is new is that `getMOSIPIntegrationFields` takes an `esignet` flag (default `true`), so existing pages behave exactly as before. Passing `esignet: false` omits the query param reader, the user info fetch, its loader and the e-Signet link button, leaving only the QR code reader. The death event's deceased page now uses it, since a deceased person cannot authenticate against e-Signet.

### Bug fixes

- Birth declarations with no child date of birth are no longer forwarded to MOSIP. The presence check never ran, because a constraint carrying a `validator` skips it and `new Date(undefined)` yields an Invalid Date that the age comparison treats as eligible.
