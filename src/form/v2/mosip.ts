...
const BIRTH_REGISTRATION_MOSIP_CONSTRAINTS = [
  /*
   * Kept separate from the age constraint below: a constraint carrying a
   * `validator` skips the presence check, so without this entry a missing
   * `child.dob` would be forwarded (`new Date(undefined)` yields an Invalid
   * Date, which the age comparison treats as eligible).
   */
  {
    fieldId: 'child.dob',
    expectedValues: 'any',
